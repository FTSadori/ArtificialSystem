...
namespace Memory
{
	...
	{
		m_full_path_str = _mark + c_disk_div + _path.full_name();
	...
	FullPath::FullPath(const std::string& _path)
	{
		...
		m_full_path_str = m_mark + c_disk_div + m_path.full_name();
		m_full_dir_name = m_mark + c_disk_div + m_path.dir();
	}

	const std::string& FullPath::full_path_str() const
	{
		return m_full_path_str;
	}
